...
class AnswerSearcher:
    def __init__(self):
        self.g = Graph(host='127.0.0.1', http_port=7474, user='neo4j', password='rdxsdzxgc')
        self.num_limit = 20
    ...
    def answer_prettify(self, question_type, answers):
        ...
        if question_type == 'related_papers':
            ...
            final_answer = 'The influence factor of {0} is {1}'.format(subject, desc)

        elif question_type == 'lfst_baseline':
            subject = answers[0]['p.name']
            desc_basname = [i['q.name'] for i in answers]
            final_answer = 'The possible baselines of {0} are {1}'.format(subject, '; '.join(list(set(desc_basname))[:self.num_limit]))
            ...
            desc = [i['q.name'] for i in answers]
            subject = answers[0]['p.name']
            final_answer = 'From KG, we find that the {0} appears in {1}.'.format(
                subject, '; '.join(list(set(desc))[:self.num_limit]))
