...
class QuestionParser:
    ...
    def sql_transfer(self, question_type, entities):
        ...
        # 查询生活方式对应的单位
            ...
            sql = [
                "MATCH (p:Lifestyle)-[r:corresponding_units]-(q:Unit) where p.name = '{0}' return p.name, q.name".format(i) for i in entities]
        ...
        # 查询生活方式的影响因子
            ...
            sql = ["MATCH (p:Lifestyle) where p.name = '{0}' return p.name, p.factor_type".format(
                i) for i in entities]

        # 查询生活方式的baseline
        elif question_type == 'lfst_baseline':
            sql = [
                "MATCH (p:Lifestyle)-[r:baselines_of_lfst]-(q:Baseline) where p.name = '{0}' RETURN p.name, q.name".format(i) for i in entities]
        ...
        # 查询生活方式属于的nation
            ...
            sql = [
                "MATCH (p:Lifestyle)-[r:corresponding_nations]-(q:Nation) where p.name = '{0}' return p.name, q.name".format(i) for i in entities]
