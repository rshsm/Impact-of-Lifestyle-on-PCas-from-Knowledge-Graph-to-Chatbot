...
class QuestionClassifier:
    def __init__(self):
        ...
        self.lfsty_wds = [
            i.strip() for i in open(
                self.lfsty_path) if i.strip()]
        self.paper_wds = [
            i.strip() for i in open(
                self.paper_path) if i.strip()]
        self.unit_wds = [i.strip() for i in open(self.unit_path) if i.strip()]
        self.pca_wds = [i.strip() for i in open(self.pca_path) if i.strip()]
        self.nation_wds = [
            i.strip() for i in open(
                self.nation_path) if i.strip()]
        self.gene_wds = [i.strip() for i in open(self.gene_path) if i.strip()]
        self.baseline_wds = [
            i.strip() for i in open(
                self.baseline_path) if i.strip()]
        self.outcome_wds = [
            i.strip() for i in open(
                self.outcome_path) if i.strip()]
        self.region_words = set(
            self.lfsty_wds +
            self.pca_wds +
            self.paper_wds +
            self.outcome_wds + self.baseline_wds)
    ...
    def classify(self, question):
        ...
        if self.check_words(
                ...
                question) and self.check_words(
                self.lead_qwds,
                question) == False and self.check_words(
                self.factor_qwds,
                question) == False and (
                'lifestyle' in types):
