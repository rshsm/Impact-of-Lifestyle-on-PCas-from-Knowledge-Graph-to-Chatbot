//定义name变量制作图标
var labels = ['Lifestyle', 'Paper', 'Nation', 'PCA_Cancer', 'Unit', 'FirClass', 'SecClass', 'ThrClass', 'Baseline', 'Outcome', 'Gene'];
var colors = ['#006BB0', '#EFA90D', '#800080', '#DC2F1F', '#795749', '#cfaca5', '#3894a6', '#a9e5bd', '#059341', '#a876d9', '#B93A85'];


//背景颜色设置 补充CSS样式设置字体布局
for (var i=0; i < labels.length; i++) {
	$('#indicator').append("<div><span style='background-color:" + colors[i] + "'></span>" + labels[i] + "</div>");
}
...
console.log(graph[0].q)

for (let item of graph) {
	for (let segment of item.p.segments) {
		// 重新更改data格式
		if (nodeSet.indexOf(segment.start.identity.low) == -1) {
			nodeSet.push(segment.start.identity.low)
			nodes.push({
		...
		if (nodeSet.indexOf(segment.end.identity.low) == -1) {
			nodeSet.push(segment.end.identity.low)
			nodes.push({
		...
		if (linkSet.indexOf(segment.relationship.start.low+','+segment.relationship.end.low) == -1){
			linkSet.push(segment.relationship.start.low+','+segment.relationship.end.low)
			links.push({
	...
	for (let segment of item.q.segments) {
		// 重新更改data格式
		if (nodeSet.indexOf(segment.start.identity.low) == -1) {
			nodeSet.push(segment.start.identity.low)
			nodes.push({
...
svgToCanvas = function (){
	saveSvgAsPng(document.getElementById("svg1"), "plot.png", {scale: 5, backgroundColor: '#9dadc1'});
};
